-- Supabase Security Fix: Enable RLS and Create Policies (Minimal Safe Version)
-- Only creates policies for tables we know the exact structure of
...
-- 2. Create RLS policies for Property table (we know this has realtorId)
...
-- 3. Create RLS policies for Session table (we know this has realtorId)
...
-- 4. Create RLS policies for CustomerProfile table (we know this has sessionId)
...
-- 5. Create RLS policies for Message table (we know this has sessionId)
...
-- 6. Create RLS policies for property_embeddings table (we know this has mls_id)
...
-- 7. Create RLS policies for Realtor table (we know this has id and isAdmin)
...
-- 8. Create RLS policies for User table (we know this has id)
...
-- 9. For PropertyEmbedding table - create a basic policy (we don't know its exact structure)
-- This will only work if the table has the expected columns
DO $$ 
BEGIN
...
EXCEPTION WHEN OTHERS THEN
    -- Table structure doesn't match our expectations, skip this policy
    RAISE NOTICE 'Skipping PropertyEmbedding policy due to structure mismatch';
END $$;

-- 10. For PropertyProximity table - create a basic policy (we don't know its exact structure)
-- This will only work if the table has the expected columns
DO $$ 
BEGIN
...
EXCEPTION WHEN OTHERS THEN
    -- Table structure doesn't match our expectations, skip this policy
    RAISE NOTICE 'Skipping PropertyProximity policy due to structure mismatch';
END $$;

-- 11. For ProximityPreference table - create a basic policy (we don't know its exact structure)
-- This will only work if the table has the expected columns
DO $$ 
BEGIN
...
EXCEPTION WHEN OTHERS THEN
    -- Table structure doesn't match our expectations, skip this policy
    RAISE NOTICE 'Skipping ProximityPreference policy due to structure mismatch';
END $$;

-- Success message
SELECT 'RLS policies created successfully! (Some policies may have been skipped due to table structure differences)' as status;
